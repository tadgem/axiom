...
namespace axm {
    ...
    class Shader {
    public:
        explicit Shader(rhi::IDevice *device, const char *name, const char *vertexEntry = "vertexMain",
    ...
    public:
        ...
        void SetData(const char *bindingName, const T &data) const {
            if (m_SlangCursor[bindingName].setData(&data, sizeof(T)) < 0) {
                AXM_LOG("Failed to set data of type {} at binding {} to pipeline with shader {}", typeid(T).name(),
                        bindingName, m_RenderPipeline->getDesc().label);
            }
        }
