...
namespace {
    ...
    void DrawGlassPanel(NVGcontext* nvg, float x, float y, float w, float h, const char* title) {
        NVGpaint shadowPaint = nvgBoxGradient(nvg, x, y + 2.0f, w, h, 8.0f, 10.0f, nvgRGBA(0, 0, 0, 120), nvgRGBA(0, 0, 0, 0));
        nvgBeginPath(nvg);
    ...
    void RenderNanoVGDemoCanvas(NVGcontext* nvg, float width, float height, float animTime, float frameMs) {
        ...
        // Background Gradient
        NVGpaint bgPaint = nvgRadialGradient(nvg, width * 0.5f, height * 0.5f, width * 0.2f, width * 0.7f,
                                             nvgRGBA(25, 30, 44, 255), nvgRGBA(10, 12, 18, 255));
        nvgBeginPath(nvg);
        ...
        char fpsStr[64];
        std::snprintf(fpsStr, sizeof(fpsStr), "%.2f ms (%.0f FPS)", frameMs, 1000.0f / (frameMs > 0.001f ? frameMs : 1.0f));
        nvgFontSize(nvg, 12.0f);
        nvgFillColor(nvg, nvgRGBA(0, 230, 180, 255));
        nvgTextAlign(nvg, NVG_ALIGN_RIGHT | NVG_ALIGN_MIDDLE);
        nvgText(nvg, width - 20.0f, 25.0f, fpsStr, nullptr);

        NVGpaint linePaint = nvgLinearGradient(nvg, 0, 50, width, 50, nvgRGBA(0, 200, 255, 255), nvgRGBA(255, 80, 180, 255));
        nvgBeginPath(nvg);
    ...
    while (init.m_Running) {
        if (engine::PreFrame(init)) {
            ...
            auto passEncoder = render_pass::BeginSwapChainRenderPass(
                init, commandEncoder, rhi::LoadOp::Clear, rhi::LoadOp::Clear, true, aml::Vec4(0.06f, 0.07f, 0.10f, 1.0f));
