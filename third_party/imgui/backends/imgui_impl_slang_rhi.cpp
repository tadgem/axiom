...
const std::string imgui_slang_shader_src = "struct VertexInput"
                                           "{"
                                           "float2 pos : POSITION;"
                                           "float2 uv  : TEXCOORD;"
                                           "float4 col : COLOR;"
...
bool              ImGui_ImplSlangRHI_Init(rhi::IDevice* device, rhi::Format renderTargetFormat) {
    ...
    g_IsSrgbTarget
            = (renderTargetFormat == rhi::Format::RGBA8UnormSrgb || renderTargetFormat == rhi::Format::BGRA8UnormSrgb
               || renderTargetFormat == rhi::Format::BGRX8UnormSrgb);

    // Load shader module at runtime
    rhi::ComPtr<slang::IBlob> diagnostics;
    slang::IModule*           module = g_Device->getSlangSession()->loadModuleFromSourceString(
            "imgui", ".", imgui_slang_shader_src.c_str(), diagnostics.writeRef());
    ...
    pipelineDesc.label                     = "ImGui Pipeline";
...
void ImGui_ImplSlangRHI_NewFrame() { g_FrameIndex = (g_FrameIndex + 1) % kMaxFramesInFlight; }

void ImGui_ImplSlangRHI_RenderDrawData(ImDrawData*              drawData,
    ...
    for (int i = 0; i < drawData->CmdListsCount; i++) {
        ...
        for (int cmdIdx = 0; cmdIdx < cmdList->CmdBuffer.Size; cmdIdx++) {
            ...
            if (pcmd->UserCallback != nullptr) {
                ...
                if (clipMinY < 0.0f)
                    clipMinY = 0.0f;
                if (clipMaxX > drawData->DisplaySize.x)
                    clipMaxX = drawData->DisplaySize.x;
                if (clipMaxY > drawData->DisplaySize.y)
                    clipMaxY = drawData->DisplaySize.y;
                if (clipMaxX <= clipMinX || clipMaxY <= clipMinY)
                    continue;

                rhi::ScissorRect scissorRect
                        = { (uint32_t) clipMinX, (uint32_t) clipMinY, (uint32_t) clipMaxX, (uint32_t) clipMaxY };
                ...
                renderState.indexFormat
                        = (sizeof(ImDrawIdx) == 2) ? rhi::IndexFormat::Uint16 : rhi::IndexFormat::Uint32;
