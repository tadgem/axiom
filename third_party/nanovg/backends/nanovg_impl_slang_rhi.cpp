...
float4 fragmentMain(VertexOutput input) : SV_Target
{
    ...
    int slangRHINVG_renderCreate(void* uptr) {
        ...
        slang::IModule*           module = device->getSlangSession()->loadModuleFromSourceString(
                moduleName.c_str(), pathName.c_str(), slang_nvg_shader_src.c_str(), diagnostics.writeRef());
        if (diagnostics) {
        ...
        if (SLANG_FAILED(device->createShaderProgram(
                    programDesc, slangCtx->shaderProgram.writeRef(), diagnostics.writeRef()))) {
            std::cerr << "NanoVG backend: Failed to create shader program" << std::endl;
            return 0;
        }

        // Create Input Layout
        rhi::VertexStreamDesc vertexStreams[] = { { sizeof(NVGvertex), rhi::InputSlotClass::PerVertex, 0 } };
    ...
    void slangRHINVG_renderFill(void*                      uptr,
                                NVGpaint*                  paint,
        ...
        if (paint->image != 0) {
            ...
            if (tex)
                frag.texType = (tex->type == NVG_TEXTURE_ALPHA) ? 2 : 1;
        }
    ...
    void slangRHINVG_renderStroke(void*                      uptr,
                                  NVGpaint*                  paint,
                                  NVGcompositeOperationState compositeOperation,
                                  NVGscissor*                scissor,
                                  float                      fringe,
    ...
    void slangRHINVG_renderTriangles(void*                      uptr,
                                     NVGpaint*                  paint,
                                     NVGcompositeOperationState compositeOperation,
                                     NVGscissor*                scissor,
...
bool nvgSlangRHICreatePipeline(NVGcontext* ctx, rhi::Format colorFormat, rhi::Format depthFormat) {
    ...
    pipelineDesc.label                   = "NanoVG Pipeline";
    ...
    for (auto& tex: slangCtx->textures) {
        if (!tex.pendingUpdates.empty() && tex.texture) {
            for (const auto& update: tex.pendingUpdates) {
                ...
                commandEncoder->uploadTextureData(tex.texture,
...
void nvgSlangRHIRender(NVGcontext*              ctx,
                       ...
                       float                    width,
                       float                    height) {
    if (!ctx || !commandEncoder || !renderPassEncoder)
        return;
